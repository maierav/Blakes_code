...
STIM.LFP_ori1  = trigData(LFP,STIM.ori1onsets,-pre,post); % this function is MLAnalysisOnline or nbanalysis. pre variable is in absolute units 
STIM.CSD_ori1  = trigData(CSD,STIM.ori1onsets,-pre,post); 
STIM.aMUA_ori1 = trigData(MUA,STIM.ori1onsets,-pre,post);

STIM.LFP_ori2  = trigData(LFP,STIM.ori2onsets,-pre,post); % this function is MLAnalysisOnline or nbanalysis. pre variable is in absolute units 
...
for c = 1:24
    ...
    area(TM,AVG.LFP_ori1(:,c)); %specify x range (refwin, mean_LFP(1 thru 351 samples, channel i)
    areafill = area(TM,AVG.LFP_ori1(:,c));
